...
@Component({
  selector: 'app-carreta',
  templateUrl: './carreta.component.html',
  styleUrls: ['./carreta.component.css']
})
export class CarretaComponent implements OnInit {
...
{"valor": "1 eixos, rodagem simples"},
{"valor": "1 eixos, rodagem dupla"},
...
public getTipo = [{"valor": "TANQUE"},
                  ...
                  {"valor": "FLORESTAL"},
                  {"valor": "BAU"},
                  {"valor": "PORTA-CONTEINER"}]

public getModelo = [{"valor": "CARRETA 1 EIXO"},
                    {"valor": "CARRETA 2 EIXO"},
                    {"valor": "CARRETA LS 3 EIXO"},
                    {"valor": "CARRETA VANDERLEIA 3 EIXOS"},
                    {"valor": "CARRETA VANDERLEIA 2 EIXOS"},
                    {"valor": "CARRETA BITREM"},
                    {"valor": "CARRETA RODOTREM"},
                    {"valor": "CARRETA CANGURU 2 EIXOS"}]
...
Placas: Array<any>;
  ...
  click_salvar() {
    ...
    if ( this.update === false ) {
         this.movemaisService.postCarreta(JsonPlaca).subscribe(resposta => {});
      } else {
         this.movemaisService.updateCarreta(JsonPlaca).subscribe(resposta => {});
       }
  ...
  click_consultar() {
      this.movemaisService.getCarretas(this.idmovemais).subscribe(dados => this.Placas = dados);
  }

  click_excluir(vplaca: string) {
    if (confirm('excluir placa : ' + vplaca)) {
            this.movemaisService.deleteCarreta(vplaca).subscribe(resposta => {});
            this.click_cancelar();
      }
  }
 ...
 const registros: Array<any> = this.jsonPlacas;
    ...
    for (i = 0; i < registros.length; i++) {
        ...
        const vJson2 = {
          ...
          this.movemaisService.postCarreta(vJson2).subscribe(resposta => {});
          this.movemaisService.updateCarreta(vJson2).subscribe(resposta => {});
    }
